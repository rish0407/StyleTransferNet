...
class Generator(nn.Module):
    def __init__(self, z_dim=4):
        ...
        self.deconv1 = nn.ConvTranspose2d(z_dim, 24, 4, stride=1, bias=False)
        self.bn1 = nn.BatchNorm2d(24)
        self.deconv2 = nn.ConvTranspose2d(24, 16, 4, stride=4, padding = 0, bias=False)
...
class Encoder(nn.Module):
    def __init__(self, z_dim=4):
        ...
        self.fc = nn.Linear(200, self.z_dim*2)
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.args.num_epochs+1):
            ...
            for input_image, output_image in Bar(self.train_loader):
                ...
                x_generated = self.G( z_fake.view(z_fake.size(0), 4, 1, 1))
                out_fake = self.D(input_image, x_generated.detach())
                ...
                ge_loss = 1.2*(g_adv + rec_loss) + 0.75*kl_loss
                ge_loss.backward()
                optimizer_ge.step()
            ...
            #     x_ge = self.G( z_out.view(z_out.size(0), 4, 1, 1))
    ...
    def save_checkpoint(self, epoch):
        if epoch == 'final':
            checkpoint_path = os.path.join(self.checkpoint_dir, 'model_new.pth')
        else:
            checkpoint_path = os.path.join(self.checkpoint_dir, f'checkpoint_new_epoch_{epoch+7}.pth')
